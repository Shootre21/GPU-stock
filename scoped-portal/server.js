...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname.startsWith('/api/executions/') && req.method === 'PATCH') {
    try {
      ...
      if (idx === -1) return send(res, 404, { error: 'Not found' });
      execs[idx] = { ...execs[idx], state: body.state || execs[idx].state, updatedAt: new Date().toISOString(), executedAt: body.state === 'executed' ? new Date().toISOString() : execs[idx].executedAt };
